...
echo "FROM frolvlad/alpine-oraclejdk8:latest" > ${DOCKER_FILE}
...
echo /usr/bin/docker run --name=${DOCKER_NAME} -e TZ="Asia/Shanghai" -e "SPRING_PROFILES_ACTIVE=local" -e "SERVER_PORT="${SPORT} -e "JAVA_OPTS=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address="${DPORT} --net host -d ${DOCKER_NAME}:${PROJ_VERSION}

/usr/bin/docker run --name=${DOCKER_NAME} -e TZ="Asia/Shanghai" -e "SPRING_PROFILES_ACTIVE="dev -e "SERVER_PORT="${SPORT} -e "JAVA_OPTS=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address="${DPORT} --net host -d ${DOCKER_NAME}:${PROJ_VERSION}
